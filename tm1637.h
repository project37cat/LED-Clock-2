...
#define DISPLAY_BRIGHT   0b00000011
...
const uint8_t chset2[20] PROGMEM = {
...
const uint8_t chcode[20] PROGMEM = {
  32,   //space
  45,   //-
  65,   //A
  67,   //C
...
//-----------------------------------------------------------------------------
uint8_t led_update(uint8_t bright)
    {
    uint8_t err=0;

    serial_begin();
    if(serial_write(DATA_COMMAND | AUTO_ADDRESS | WRITE_DATA)) err|=0x01;
    serial_end();

    serial_begin();
    if(serial_write(ADDRESS_COMMAND | DEFAULT_ADDRESS)) err|=0x02;
    ...
    serial_begin();
    if(serial_write(DISPLAY_COMMAND | DISPLAY_ON | (bright & 0b00000111))) err|=0x04;
    serial_end();
...
void led_char(uint8_t pos, uint8_t code)
    {
    uint8_t tmp = pgm_read_byte(&chset1[10]);  //for unsupported chars
...
//-----------------------------------------------------------------------------
void led_dat(uint8_t pos, uint8_t dat)
    {
    if(pos<4) sbuff[pos]=(dat|(sbuff[pos]&0b10000000));
    }


//-----------------------------------------------------------------------------
void led_print(uint8_t x, char *str)  //print a string  //x: 0..3
    {
    for(;((*str) && (x<4));) led_char(x++,*str++);
    }
...
void led_dots(uint8_t on)
    {
    if(on) { SET_BIT(sbuff[1],7); } else { CLR_BIT(sbuff[1],7); }
    }
